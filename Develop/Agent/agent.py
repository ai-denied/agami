...
def process_threat(ip, fail_count, challenge_id):
    ...
    # ==========================================
    prompt = f"""
    당신은 Agami 캡챠 서비스의 보안 관제 에이전트입니다.
    다음은 비정상적인 캡챠 실패를 유발한 트래픽의 정보입니다.

    - 공격 의심 IP: {ip}
    - 1분간 실패 횟수: {fail_count}회
    - 사용자 마우스/클릭 행동 로그(JSON 추출): {behavior_data}

    이 데이터를 바탕으로, 이것이 단순한 사용자의 실수인지 아니면 매크로 봇에 의한 악의적 우회 시도인지 짧게 분석하고, 보안 담당자가 즉각 조치할 수 있도록 한국어로 요약된 경고 보고서를 작성해주세요.
    """
