...
const Price = () => {
  ...
  const handlePlanClick = async (plan) => {
    ...
    if (planName === "enterprise") {
      showAlert("엔터프라이즈 요금제는 현재 준비 중입니다. 고객센터로 문의해주세요.");
      return;
    }

    if (planHierarchy[planName] < planHierarchy[currentPlan]) {
      showAlert("현재 이용 중인 요금제보다 하위 요금제로 다운그레이드할 수 없습니다.");
      return;
    }
    if (planName === currentPlan) {
      showAlert("이미 이용 중인 요금제입니다.");
      return;
    }
  ...
  const pricingPlans = [
    {
      name: "Basic",
      description: "개인 프로젝트 및 소규모 웹사이트를 위한 완벽한 시작",
      price: "0",
      features: ["월 10,000건 인증 제공", "손전등 캡챠 모델 이용 가능", "기본 트래픽 분석 대시보드", "1개 도메인 등록", "커뮤니티 기술 지원"],
      buttonText: "무료로 시작하기"
    },
    {
      name: "Pro",
      description: "성장하는 비즈니스와 전문 개발자를 위한 강력한 성능",
      price: "49,000",
      features: ["월 100,000건 인증 제공", "전체 캡챠 모델 무제한 제공", "심층 트래픽 및 공격 유형 분석", "최대 5개 도메인 등록", "우선 이메일 기술 지원"],
      buttonText: "Pro 플랜 선택"
    },
    {
      name: "Enterprise",
      description: "대규모 트래픽과 최고 수준의 보안이 필요한 기업",
      price: "문의",
      features: ["맞춤형 인증 한도 제공", "SLA 보장 및 전담 엔지니어", "커스텀 캡챠 모델 구축", "무제한 도메인 등록 지원", "24/7 연중무휴 핫라인 지원"],
      buttonText: "도입 문의하기"
    }
  ];

  // 💡 [핵심] 스크롤 이벤트 감지하여 닷(Dot) 활성화 인덱스 업데이트
  const handleScroll = () => {
    ...
    <div className="pricing-wrapper">
      ...
      <div className="pricing-content">
        <LiquidGlass style={{ padding: "50px 40px" }}>
          ...
          {/* 💡 [핵심] 모바일 전용 슬라이드 인디케이터 렌더링 (PC에서는 안 보임) */}
